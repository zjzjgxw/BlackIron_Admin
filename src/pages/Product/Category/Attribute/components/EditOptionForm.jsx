...
const EditOptionForm = (props) => {
  const { modalVisible, onCancel, onDelete, onAdd, attribute } = props;

  const [inputVisible, setInputVisible] = useState(false);

  const [inputValue, setInputValue] = useState('');

  const [tags, setTags] = useState(attribute.options);
  ...
  const handleSaveOptions = async (value) => {
    const res = await onAdd({ attributeId: attribute.id, content: value });
